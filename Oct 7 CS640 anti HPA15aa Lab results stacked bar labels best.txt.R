...
#Titre range was changed to 1 to 16 for not detected to 1 in 8.  Axis labels can be changed later
Plate.DF<-read_csv("CS640 anti-HPA15aa titres_2.csv")
...
Titre_range<-c(1,2,4,8, 16)
Res.DF$text.x<-Titre_range[g_g1.bld$x+1]
...
Res.DFc<-Res.DFc  %>% mutate(Labn_rev = rev(Lab_number)) # reverse the lab numbers
...
g_ghisc<-ggplot(Res.DFc, aes(Titre, fill = as.factor(Assay_number), group = nn))+
  scale_x_continuous(trans = 'log2', limits = c(0.7, 128), breaks = c(1,2,4,8,16,32, 64, 128), 
                     labels = c("not detected", "neat", "1 in 2", "1 in 4", "1 in 8", "1 in 16", "1 in 32", "1 in 64"))+ #x axis settings
  scale_y_continuous(limits = c(0, 35), breaks = c(0, 5, 10, 15, 20, 25, 30, 35), expand = c(0,0))+ #y axis settings
  geom_histogram(binwidth = 1, colour="black")+ #histogram settings
  scale_fill_grey(start = 1, end = 0.85, name = "Assay number")+ # fill colour settings with adjustable grey scale
...
#Make some tiffs
tiff("CS640 HPA15aa 3.tif", units="in",width=11,height=9.0,res=300,compression='lzw')
g_ghisc
dev.off()
